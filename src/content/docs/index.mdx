...
- Non-deterministic (same question ≠ same answer)
...
- ✅ **Deterministic** - same spec always produces the same code
- ✅ **Verified** - must pass tests before it can be used
- ✅ **Reproducible** - cryptographic hashing guarantees consistency
- ✅ **Importable** - no copy-paste, just regular Python imports
- ✅ **Auditable** - every checkpoint has metadata and hashes
...
def fibonacci(n: int) -> int:
    ...
    yield VibesafeHandled()  # AI fills in everything after this
```

```bash
# 2. Compile (calls LLM, generates implementation)
$ vibesafe compile --target app.math/fibonacci
✓ Extracted spec
...
$ vibesafe test --target app.math/fibonacci
✓ Doctest 1/3 passed
✓ Doctest 2/3 passed
✓ Doctest 3/3 passed
...
$ vibesafe save --target app.math/fibonacci
✓ Checkpoint activated
```

```python
# 5. Use it (just regular imports)
from app.math import fibonacci
...
**Before Vibesafe:**
```
Write code → Ask AI → Copy-paste → Manual review → Hope → Deploy → 🔥
```

**With Vibesafe:**
```
Write spec → vibesafe compile → vibesafe test → git commit → Deploy → ✅
```
